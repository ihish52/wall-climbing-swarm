...
//from slave code
extern float allxposi[4], allyposi[4], allerro[4], allheadi[4], testvalsi[4], debugsi[8];
extern char x;
...
void recfromslave(const uint8_t * mac_addr, const uint8_t *incomingData, int len) {
  ...
  //updating data
	...
	send_slave.testvals[rec_slave.id] = rec_slave.test_val; 
	send_slave.debugs[rec_slave.id] = rec_slave.test1; 
	send_slave.allypos[rec_slave.id + 4] = rec_slave.test2;
...
void master_send(float err, float heading, float xpos, float ypos)
{
  ...
  if(millis() - timer_master > SEND_DELAY){  //75 
...
//  Serial.println("rec");
  ...
  memcpy(testvalsi, rec_master.testvals, sizeof(rec_master.testvals));
  memcpy(debugsi, rec_master.debugs, sizeof(rec_master.debugs));
...
void slave_send(int id, float err, float heading, float xpos, float ypos)
{
  ...
  send_master.test_val = test_val;
  send_master.test1 = test1;
  send_master.test2 = test2;

  if(millis() - timer_slave > SEND_DELAY) {
...
void controller_send() {
  ...
  if(millis() - timer_cont > SEND_DELAY) {
    esp_err_t result = esp_now_send(masterAddress, (uint8_t *) &send_master, sizeof(send_master));
    timer_cont = millis();
